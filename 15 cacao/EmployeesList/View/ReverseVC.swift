...
//  15 cacao
//
//  Created by Мария Хатунцева on 07.05.2022.
//
    ...
    @objc private func filterApply(_ segment: UISegmentedControl) -> Void {
        ...
        @objc func goToApplicationForm(_ sender: Any) {
            if let vc = ViewController() as? ViewController {
                vc.employee = Employee.init(entity: NSEntityDescription.entity(forEntityName: "Employee", in: persistentContainer.viewContext)!, insertInto: persistentContainer.viewContext)
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
    override func numberOfSections(in tableView: UITableView) -> Int {
            return fetchedResultsController1.sections?.count ?? 0
        }
       override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            let employee = fetchedResultsController1.object(at: indexPath)
        ...
        override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            if let vc =  ViewController() as? ViewController {
                vc.employee = fetchedResultsController1.object(at: indexPath)
                navigationController?.pushViewController(vc, animated: true)
            }
    }
    }
    extension ReverseVC: NSFetchedResultsControllerDelegate {
